...
gen_device = 0    # GPU device for generation (使用第一个 GPU)
beta = 0.04
all_steps = 1000
Q_batch_size = 5
num_pre_Q = 8
...
# Training configuration (replacing ds_config)
learning_rate = 1e-6
...
def GRPO_step(batch, model):
    ...
    if 'gen_logps' in batch:
        ratio = torch.exp(per_token_logps - batch['gen_logps'].to(device))
...
if __name__ == '__main__':
    ...
    # Optional: Setup gradient scaler for mixed precision (fp16)
